...
export default function VendorPaymentRequestsPage() {
  ...
  // Get completed orders for summary tab
  const completedOrders = mockVendorOrders
    ...
    .map((order) => ({
      ...
      items: order.items,
      hasPaymentRequest: paymentRequests.some((pr: any) =>
        pr.items?.some((item: any) => item.orderId === order.id)
      ),
    }))
    .filter((order) => !order.hasPaymentRequest);

  // Summary data for PaymentRequstSummy component
  const summaryData = {
    total: paymentRequestStats?.data?.attributes?.totalRequests || 0,
    pending: paymentRequestStats?.data?.attributes?.pendingRequests || 0,
    paid: paymentRequestStats?.data?.attributes?.approvedRequests || 0,
    rejected: 0, // You might need to add this to your API
    totalAmount: paymentRequestStats?.data?.attributes?.availableWithdrawl || 0,
  };
